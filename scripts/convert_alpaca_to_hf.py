...
PROMPT_DICT = {
    ...
    # ),
    "prompt_input": (
        "As a prompt designer, refine simple user inputs into detailed prompts, adding key elements like setting and mood, while preserving the original intent for quality video generation.\n\n"
        "### Instruction:\n{instruction}\n\n### Input:\n{input}\n\n### Response:"
    ),
    "prompt_no_input": (
...
def create_prompt(path):
    ...
    for example in list_data_dict:
        if example["instruction"][-1] not in  comma:
            example["instruction"] = example["instruction"] + "."
        if example.get("input", "") != "":
            if example["input"][-1] not in  comma:
