...
if __name__ == "__main__":
    ...
    parser.add_argument('--refer-file','-rf', type=str, required=True, help='refer file')
    ...
    num_added_toks = tokenizer.add_special_tokens(
        {
            "additional_special_tokens": ["<GOGOAC>"],
        }
    )
    # logger.info("We have added {} tokens".format(num_added_toks))

    # num_added_toks = tokenizer.add_tokens(["<acacac>"])
    
    embedding_size = model.get_input_embeddings().weight.shape[0]
    if len(tokenizer) > embedding_size:
        model.resize_token_embeddings(len(tokenizer))
    
    newtokenid=tokenizer.convert_tokens_to_ids("<GOGOAC>")
    oldtokenid=tokenizer.convert_tokens_to_ids(".")
    oldtokenid=tokenizer.convert_tokens_to_ids(tokenizer.pad_token)

    print(newtokenid, oldtokenid)
    print('-',model.get_input_embeddings().weight[newtokenid, :].data)
    print('-',model.get_input_embeddings().weight[oldtokenid, :].data)
    # print(model)

    # model.model.embed_tokens.weight[newtokenid, :].data = model.model.embed_tokens.weight[oldtokenid, :].data

    with torch.no_grad():
        # model.model.embed_tokens.weight[newtokenid, :] = model.model.embed_tokens.weight[oldtokenid, :]
        model.get_input_embeddings().weight.data[newtokenid, :] = model.get_input_embeddings().weight.data[oldtokenid, :]
        model.get_output_embeddings().weight.data[newtokenid, :] = model.get_output_embeddings().weight.data[oldtokenid, :]



    print(model.model.embed_tokens.weight[newtokenid, :].data)
    print(model.model.embed_tokens.weight[oldtokenid, :].data)

 
    # with torch.no_grad():
    #     model.model.embed_tokens.weight[newtokenid, :] *= 0
    ...
    print(model.get_output_embeddings().weight[newtokenid, :])
    print(model.get_output_embeddings().weight[oldtokenid, :])

    print(model.get_input_embeddings().weight[newtokenid, :])
    print(model.get_input_embeddings().weight[oldtokenid, :])

    model.get_input_embeddings().weight[oldtokenid, :] *= 0
    ...
    output_data = read_input(refer_file)
    prompt = create_prompt(instruct, input_data, output_data, template)
