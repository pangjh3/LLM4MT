...
class DataTrainingArguments:
    ...
    training_data_num_lines: Optional[int] = field(
        default=-1,
        metadata={
            "help": "The lines for training. -1 for all."
        },
    )
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.training_data_num_lines != -1:
            raw_datasets["train"] = load_dataset(
                extension,
                data_files=data_files,
                split=f"train[0:{data_args.training_data_num_lines}]",
                cache_dir=model_args.cache_dir,
                use_auth_token=True if model_args.use_auth_token else None,
                streaming=data_args.streaming,  # xxx (2023-09-20): add streaming for local data
                **dataset_args,
